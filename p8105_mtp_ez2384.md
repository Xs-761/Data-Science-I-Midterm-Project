...
- Identify and Solve Issues arise when merging.
  - When merging the datasets, a warning message appears: Detected an
    unexpected many-to-many relationship between x and y variables. This
    implies that the merging key “zipcode” is not unique in the
    `NYC_Zipcode` dataset.  
  - Upon closer checking, zipcode are all unique in `NYC_Rental_Price`
    and `NYC_Housing Price` datasets, but contain duplicate values 10463
    and 11201 in the `NYC_Zipcode` dataset. Remove the incorrect records
    leave us with the correct records 11201–Kings–Brooklyn &
    10463–Bronx–Bronx.  
...
- Data Cleaning Processes
  - Misleading Column Names
    - In both `NYC_Rental_Price` and `US_Housing_Price` datasets,
      changed `RegionName` to `zipcode`.  
  - Convert Dataframe Layout
    - Converted `NYC_Rental_Price` and `NYC_Rental_Price` to a longer
      format with all date variables listed under column `date` and
      values listed under column `rental_price` and `housing_price`,
      respectively.  
...
- The reason why there is 116 months between Jan.2015 to Aug.2024 but
  there are much fewer months in the `NYC_Rental_Price` dataset is
  likely because:
  - The company considered some of the Months within this period do not
    have sufficiently meaningful research values for collection.  
  - The company decided not to include some of the Months as the rental
    prices data are too incomplete within this period.  
- The reason why the number of zipcodes in the `NYC_Rental_Price`
  dataset is 149 whereas that in the `NYC_Zipcode` dataset is 320 is
  likely due to:
  - The company considered not necessary to include rental price data
    for every single zipcode within NYC.  
- The following Table1 shows the average rental price in each borough
  and year:
...
- Plot2 Remarks
  - CA shows the highest house prices with some zipcodes exceeding \$6
    million, whereas WV has the least.  
