...
###### Problem1 Discussion   
- Identify and Solve Issues arise when merging.   
  - When merging the datasets, a warning message appears: Detected an unexpected many-to-many relationship between x and y variables. This implies that the merging key "zipcode" is not unique in the `NYC_Zipcode` dataset.   
  - Upon closer checking, zipcode are all unique in `NYC_Rental_Price` and `NYC_Housing Price` datasets, but contain duplicate values 10463 and 11201 in the `NYC_Zipcode` dataset. Remove the incorrect records leave us with the correct records 11201--Kings--Brooklyn & 10463--Bronx--Bronx.   
...
###### Problem2 Discussion    
- The reason why there is 116 months between Jan.2015 to Aug.2024 but there are much fewer months in the `NYC_Rental_Price` dataset is likely because:    
  - The company considered some of the Months within this period do not have sufficiently meaningful research values for collection.    
  - The company decided not to include some of the Months as the rental prices data are too incomplete within this period.    
- The reason why the number of zipcodes in the `NYC_Rental_Price` dataset is `r n_distinct(NYC_Rental_Price$zipcode)` whereas that in the `NYC_Zipcode` dataset is `r n_distinct(NYC_Zipcode$zipcode)` is likely due to:   
  - The company considered not necessary to include rental price data for every single zipcode within NYC.    
- The following Table1 shows the average rental price in each borough and year:    
...
- Remarks on Table1   
  ...
  - For Staten Island, the data is missing from 2015 to 2019. From 2020 onward, values are recorded.    
  - Price variation between boroughs is significant: Manhattan consistenty has the highest rental prices while the Staten Island has the lowest prices(for available years).    
...
- Remarks on Table2   
  - For most zipcodes, rental prices decreased from Jan.2020 to Jan.2021.   
- The following Table3 gives the largest drop in price from Jan.2020 to Jan.2021 with neighborhoods included:   
`r knitr::kable(Table3)`    
- Remarks on Table3   
  - For all available boroughs and among those neighborhoods with the highest price drop from Jan.2020 to Jan.2021, `Lower Manhattan` has the largest price decrease of `$912.60` and `Southeast Bronx` has the least price decrease of `$16.46`.   
  - Rental prices in Bronx as a whole did not drop significantly between Jan.2020 to Jan.2021.    
  - Staten Island data in both Jan.2020 and Jan.2021 are unavailable for analysis.  
...
- Plot1 Remarks   
  - Large Geographic Rental Disparities, with Manhattan having the highest rental prices while Bronx has relatively the lowest.  
  - General upward rental price trend across all boroughs especially in Manhattan and Brooklyn.   
  - Impact of COVID-19 is clear as there is a salient drop in rental prices across most boroughs in pandemic-severe time periods.   
...
- Plot2 Remarks   
  - CA shows the highest house prices with some zipcodes exceeding $6 million, whereas WV has the least.    
...
- Plot3 Remarks   
  - General positive correlation between housing prices and rental prices by zipcode.
